...
class ClientUserMaker(BaseClientUserMaker):
    ...
    async def recv(cls, client: "distkv.client.Client", ident:str, _kind:'user'):
        ...
        async with client.stream(action="auth_get",typ=cls._auth_method,kind=_kind,ident=ident, stream=True) as s:
            m = await s.recv()
    ...
    async def send(self, client: "distkv.client.Client", _kind='user'):
        ...
        async with client.stream(action="auth_set",typ=type(self)._auth_method,kind=_kind, stream=True) as s:
